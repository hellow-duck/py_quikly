...
class CandleFunctions(BaseFunctions):
    ...
    async def get_candles_pandas(self, class_code: str, sec_code: str, interval, count: int = 100):
        """
        Получить свечи и вернуть как структурированный numpy массив.
        
        Args:
            class_code (str): Код класса
            sec_code (str): Код инструмента
            interval: Интервал свечей
            count (int): Количество свечей
        
        Returns:
            numpy.ndarray: Структурированный массив с полями:
                datetime, open, high, low, close, volume
        """
        import numpy as np
        from datetime import datetime
        
        candles = await self.get_last_candles(class_code, sec_code, interval, count)
        ...
        data = np.empty(len(candles), dtype=dtype)
        
        for i, c in enumerate(candles):
            dt = c.datetime
            data[i]['datetime'] = np.datetime64(
                f"{dt.year:04d}-{dt.month:02d}-{dt.day:02d}T{dt.hour:02d}:{dt.min:02d}:{dt.sec:02d}"
            )
            data[i]['open'] = c.open
            data[i]['high'] = c.high
            data[i]['low'] = c.low
            data[i]['close'] = c.close
            data[i]['volume'] = c.volume

        import pandas as pd

        dates_ns = data['datetime'].astype('datetime64[ns]')
