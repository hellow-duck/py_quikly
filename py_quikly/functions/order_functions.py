...
class OrderFunctions(BaseFunctions):
    ...
    async def send_order(self, class_code: str, security_code: str, account_id: str,
        ...
        try:
            ...
            if isinstance(operation, str):
                # Если пришла строка
                if operation.upper() in ['B', 'BUY', 'BUY']:
                    new_order_transaction.OPERATION = TransactionOperation.B
                else:
                    new_order_transaction.OPERATION = TransactionOperation.S
            else:
                # Если пришел enum Operation
    ...
    async def send_limit_order(self, class_code: str, sec_code: str, account_id: str,
                            operation: Operation, price: Decimal, quantity: int,
                            execution_condition: ExecutionCondition = ExecutionCondition.PUT_IN_QUEUE,
                            client_code: Optional[str] = None) -> Order:
        """
    ...
    async def send_market_order(self, class_code: str, sec_code: str, account_id: str,
                            operation: Operation, quantity: int,
                            execution_condition: ExecutionCondition = ExecutionCondition.PUT_IN_QUEUE,
                            client_code: Optional[str] = None) -> Order:
        """
    ...
    async def cancel_order(self, order_num, class_code: str, sec_code: str) -> int:
        """
        Отмена заявки.
        ...
        Args:
            ...
            Args:
            order_num: Номер заявки, которую требуется отменить
            class_code: Код класса инструмента
        ...
        kill_order_transaction.CLASSCODE = class_code
        kill_order_transaction.SECCODE = sec_code
        kill_order_transaction.ORDER_KEY = str(order_num)
